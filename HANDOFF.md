...
Tento dokument je určený pre ďalšiu Claude session (alebo iného agenta), aby vedela kontext projektu a pracovné konvencie. Posledná session: 2026-05-05.
...
- **Dva varianty UI v jednom repe:**
  - **PC verzia** — koreň repa (`konfigurator.jsx`, `index.html`)
  - **Mobile verzia** — `phone/` subfolder (`phone/konfigurator.jsx`, `phone/index.html`) — **rovnaké pravidlá, ceny, validácie, CONFIG ako PC; UI prispôsobené na telefón**
- **Lokálny pracovný priečinok:** `C:\Users\M\Desktop\claud\konfigurator V4`
- **Aktuálny commit:** `be0c3f7` (pridaná mobile verzia v phone/)
- **Live URL:**
  - PC: `https://raw.githack.com/luxusnerohoze-dev/konfigurator/master/index.html`
  - Mobile: `https://raw.githack.com/luxusnerohoze-dev/konfigurator/master/phone/index.html`
...
| `phone/` | **Mobile verzia** — kompletná kópia všetkých súborov (jsx, html, compile, package, images), s upraveným UI pre mobil. Build pipeline beží lokálne v `phone/`. |
...
| `26d8f30` | Pridaný HANDOFF.md (tento dokument) |
| `be0c3f7` | **Pridaná mobile verzia** v `phone/` subfolderi — kompletne 1:1 pravidlá, ceny a CONFIG s PC verziou; UI prispôsobené na telefón (single-column, sticky bottom bar, väčšie tap targets, kompaktnejšia galéria, `viewport-fit=cover`) |

---

## 6.5 Mobile verzia (`phone/` subfolder)

**Pravidlá pre mobile (zopakované, dôležité):**

- `phone/konfigurator.jsx` má **identické** `CONFIG`, `PRICES`, `MATERIALS`, `LEMOVANIE`, `NASIVKY`, `NITE_FARBY`, `FIELD_META`, `FIELD_ORDER`, `MATERIAL_DESCRIPTIONS`, `calculatePrice` ako PC verzia. **Validácie, scroll triggery, auto-advance, bundle pravidlá — všetko 1:1.**
- **Žiadne business-logic divergencie** medzi PC a mobile. Ak Michal pridá nový kamión, novú cenu alebo nové pravidlo, **musí to zmeniť v oboch súboroch** (`konfigurator.jsx` aj `phone/konfigurator.jsx`). Inak je verzia nesyncovaná.
- **Single source of truth pre konštanty by bol lepší** (extract do shared file), ale aktuálne to je samostatná kópia kvôli jednoduchosti deploymentu (raw.githack mapuje súbory 1:1 z repa, nepotrebuje build).

**Mobile-špecifické UI rozdiely vs PC:**

| Aspekt | PC | Mobile |
|---|---|---|
| Container | `maxWidth: 1280, padding: 20` | `maxWidth: 480, padding: 12 12 92 12` (92px bottom kvôli sticky bar) |
| Layout | 2-stĺpcový (`konfig-left` 420px sticky + `konfig-right` 480px) | Single column (`flex-direction: column`) |
| Header | LCD 13px, h1 24px, h2 18px | LCD 10px, h1 18px, h2 12px |
| Galéria fotka | aspect 4/3, šípky 36px | aspect 16/10, šípky 44px (touch-friendly) + bullet indicator pozície |
| Akordeón header | padding 14×20 | padding 16×16, `minHeight: 48` (Apple HIG tap target) |
| Dropdown | padding 12×40×12×14, fontSize 14 | padding 14×44×14×14, fontSize 15, `minHeight: 28` |
| Sticky bottom bar | žiadny | **CELKOM cena + "🛒 Do košíka" tlačidlo** prilepené dole (`position: fixed`) — klik scrolluje na hlavné Pridať do košíka tlačidlo a auto-klikne |
| Hlavné Pridať do košíka tlačidlo | inline 18×56 | full-width 16×32 (`width: 100%`), označené `data-pridat-button="true"` aby sticky bar mohol nájsť |
| Viewport meta | default | `viewport-fit=cover`, `theme-color #C5A44E`, `apple-mobile-web-app-capable` |
| Tap behavior | default | `-webkit-tap-highlight-color: transparent`, `-webkit-touch-callout: none` |
...
cd phone
node compile_jsx.js
# regeneruje phone/index.html, phone/konfigurator_preview.html, phone/konfigurator_compiled.js
```

`phone/compile_jsx.js` je identická kópia z PC. Babel rezolvuje `@babel/core` z `../node_modules/` (parent), takže netreba samostatný `npm install` v `phone/`.

**Workflow keď Michal pridá zmenu (typicky):**

1. Edit PC `konfigurator.jsx` (pravidlá / UI)
2. Run `node compile_jsx.js` v koreni
3. **Ak je zmena v pravidlách (CONFIG, PRICES, validácie, calculatePrice):** porty 1:1 do `phone/konfigurator.jsx` (nájdi rovnaký blok cez `grep -n` a copy). Run `node phone/compile_jsx.js`.
4. **Ak je zmena len v UI:** mobile sa dotkne iba ak treba (nie všetky UI zmeny majú zmysel na mobile).
5. Commit, push.
...
- **PC ↔ Mobile sync je manuálny.** Každá zmena v pravidlách musí ísť do oboch súborov. Najlepší follow-up: extrahovať `CONFIG`, `PRICES`, `MATERIALS`, `LEMOVANIE`, `NASIVKY`, `NITE_FARBY`, `MATERIAL_DESCRIPTIONS`, `calculatePrice` do samostatného `shared.js` a oba `konfigurator.jsx` (PC aj phone) ho `import`ujú. Vyžaduje malý refaktor build pipeline (compile musí inline-ovať shared.js do compiled výstupu).
- **Phone deployment v separátnom Windows priečinku** (`C:\Users\M\Desktop\claud\konfigurator V4 phone`): Michal hovoril že má separátny folder na disku. Aktuálne phone verzia žije v `phone/` subfolderi PC repa. Ak chce mať aj separátny mirror na disku, môže použiť tento bat skript:
  ```
  @echo off
  xcopy /E /I /Y "C:\Users\M\Desktop\claud\konfigurator V4\phone" "C:\Users\M\Desktop\claud\konfigurator V4 phone"
  ```
  Alebo lepšie — separátny mirror nie je potrebný, lebo raw.githack URL `.../phone/index.html` slúži priamo zo subfolderu.
